...
cube_center = [3 3 3];   % centroid of the 4x4x4 cube, now drawn at DNA coords 1..4
piece_patches = cell(13,1);   % piece_patches{pid} = array of patch handles

% The 4x4x4 Bedlam cube is the INTERIOR of the 5x5x5 DNA design space:
% Bedlam local index (ix,iy,iz)=0..3  ->  DNA coordinate (dx,dy,dz)=1..4.
for iz = 0:3
    for iy = 0:3
        for ix = 0:3
            dx = ix+1;  dy = iy+1;  dz = iz+1;   % DNA coordinate (interior)
            pid   = piece_id(ix+1, iy+1, iz+1);
            color = piece_colors(pid, :);
            k     = dx + 5*dy + 25*dz;
            lines = make_info_lines(dx,dy,dz,k,pid,piece_names{pid},strand_db);
            new_patches = draw_cube(ax, dx,dy,dz, color, C.edge, pid, lines);
...
lighting(ax,'gouraud');
view(ax, 35, 25);
xlim(ax,[-4.5 10.5]); ylim(ax,[-4.5 10.5]); zlim(ax,[-4.5 10.5]);
...
%% =================================================================
function lines = make_info_lines(dx,dy,dz,k,pid,pname,strand_db)
% dx,dy,dz are DNA coordinates (1..4) — the 4x4x4 interior of the 5x5x5 space.
face_note = {'','','','','','',''};
...
% Single sheet, no header. Col 1 = sequence, Col 2 = "Strand X" name.
...
% Read a sheet robustly. sheet may be a name (char) or index (numeric).
...
% Compute centroid of each piece, in DNA coordinates (drawn at 1..4).
% data is 0-indexed Bedlam-local, so cell center = data + 1 (offset) + 0.5.
piece_centroids = zeros(13,3);
piece_count     = zeros(13,1);
for r = 1:size(data,1)
    pid = data(r,4);
    piece_centroids(pid,:) = piece_centroids(pid,:) + [data(r,1)+1.5, data(r,2)+1.5, data(r,3)+1.5];
